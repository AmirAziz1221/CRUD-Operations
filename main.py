...
fake_users_db = {
    "johndoe": {"username": "johndoe", "password": "secretpassword", "role": "user"},
    "adminuser": {"username": "adminuser", "password": "adminpass", "role": "admin"}
}
...
# Create JWT token
def create_jwt_token(data: dict, role: str):
    to_encode = data.copy()
    to_encode.update({
        "role": role,
...
async def login(user: Login):
    ...
    role = db_user["role"]
    token = create_jwt_token(token_data, role=role)
    return {"access_token": token, "token_type": "bearer"}

# Protected route
@app.get("/protected", tags=["Protected"])
async def protected_route(current_user: dict = Depends(get_current_user)):
    return {"message": f"Hello, {current_user['sub']}! You are authenticated as {current_user['role']}."}
